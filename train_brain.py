...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--algorithm_category', dest='algorithm_category', help='choose VIModel:0 or SVIModel:1',
                        default=0)
    parser.add_argument('-name', '--data_name', dest='data_name', help='data_name', default='0_haxby')
    ...
    # cp = ClusterProcess(model=VmfHMM(args), n_components=args.N*args.K, smoothing_fwhm=6.,
    ...
    # ca = np.unique(pred)
    # print(ca)
    # console_log(pred=pred[:2000], data=train_data[:2000], model_name='HMM-VMF-brain')
